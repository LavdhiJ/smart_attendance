...
export const unrecognizedService = {
  ...
  getUnrecognizedFaces: async (filters = {}) => {
    ...
    const response = await axios.get(`/api/unrecognized-faces?${params}`);
  ...
  getUnresolvedCount: async (classSubjectId) => {
    ...
    const response = await axios.get(`/api/unrecognized-faces/count${params}`);
    return response.data;
  },

  // Get recent unresolved
  getRecentUnresolved: async (days = 7, limit = 50) => {
    const response = await axios.get(`/api/unrecognized-faces/recent?days=${days}&limit=${limit}`);
    return response.data;
  },
  ...
  getByDate: async (date, classSubjectId) => {
    ...
    const response = await axios.get(`/api/unrecognized-faces/by-date?${params}`);
    return response.data;
  },

  // Resolve face
  resolveFace: async (faceId, studentId) => {
    const response = await axios.post(`/api/unrecognized-faces/${faceId}/resolve`, { student_id: studentId });
    return response.data;
  },

  // Bulk resolve
  bulkResolve: async (faceIds, studentId) => {
    const response = await axios.post('/api/unrecognized-faces/bulk-resolve', {
      face_ids: faceIds,
  ...
  deleteFace: async (faceId) => {
    const response = await axios.delete(`/api/unrecognized-faces/${faceId}`);
    return response.data;
  },

  // Cleanup old resolved
  cleanupOldResolved: async (days = 90) => {
    const response = await axios.post(`/api/unrecognized-faces/cleanup?days=${days}`);
    return response.data;
  },
};
